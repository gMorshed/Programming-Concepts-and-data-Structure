...
def lex(code):
    ...
    ca  n. This means you can have symbols like this:
    ...
    for m in p.finditer(code):
        ...
        if(m.group(1)):
            ...
            yield Symbol(m.group(8))

        elif(m.group(9)):
            yield parse_strlit(m.group(9))
...
def parse_strlit(tok):
    ...
    while(i < len(tok) - 1):
        ...
        if(i + 3 < len(tok) and tok[i] == '\\' and tok[i + 1] == 'x'):

            p = re.compile(r'[0-9a-fA-F]{2}')

            temp = tok[i + 2] + tok[i + 3]

            if(bool(p.fullmatch(temp))):

                result = result + (chr(int(temp, 16)))
        ...
        elif(i + 3 < len(tok) and tok[i] == '\\' and tok[i + 1] == '0'):

            p = re.compile(r'[0-7]{2}')
            ...
            if(bool(p.fullmatch(temp))):
                ...
                result = result + (chr(int(temp, 8)))

                i = i + 4

            elif(i + 1 < len(tok) and tok[i] == '\\'):

                if(tok[i + 1] in map_string):

                    result = result + (chr(map_string[tok[i + 1]]))
...
class QuoteParenthesisError(Exception):
...
def parse(tokens):
    ...
    while(True):
        ...
        for index, x in enumerate(list_of_token):

            if (isinstance(x, RParen)):
        ...
        while(index >= 0):
            ...
            # as soon as I find the left paren, I break out of the loop
            if isinstance(list_of_token[index], LParen):
            ...
            index = index - 1
        # doesn't work if I have both left and right parenthesis
        if(left_index == 999):

            break

        temp_token = list_of_token[left_index + 1:right_index]

        del list_of_token[left_index:right_index]

        # returns an SExpression and replace it for the indexes in the actual
        # list
        list_of_token[left_index] = helper_function(temp_token)

    is_quote = False

    count_quote = 0

    for item in list_of_token:

        if isinstance(item, SExpression):
        ...
        elif isinstance(item, type(QuoteParenthesisError)):

            raise SyntaxError('invalid quotation')

        elif isinstance(item, LParen):

            raise SyntaxError('incomplete parse')

        elif isinstance(item, RParen):
...
def helper_function(l):
    ...
    if(is_quote):
        ...
        return QuoteParenthesisError

    else:

        return SExpression.from_iterable(a)
