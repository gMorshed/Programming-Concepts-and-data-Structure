...
:Implemented By:
   * Jeff Greene <jbgreene@mymail.Mines.EDU>
   * Gazi Mahbub Morshed <gmdmahbubmorshed@mymail.Mines.EDU>
   * Matt Goin <mlynngoin@mymail.Mines.EDU>

Deliverable 7: We used prompt toolkit to make the REPL nicer. Right now 
it has autocomplete feature, both for the buildin function's name and for some 
words that could come up while writing. It has syntax highlighting feature and 
the up and down key arrow can be used to access history of the command used
in this session.
We used roughly 7-8 hours trying out some of the suggested features but 
unfortunately this is only that we got to work fully.




Language extensions for D6: We implimented default argument and keyword argument feature for our language extension. As there is no rigid Syntax specified for the Language 
extension, we made our Syntax similar to python's. But in our case, when specifying the default or keyword argument, there can not be any white space in between the 
varioable name, equal sign and the value (e.g my_var=45, or for a predefied variable called pre_var, it has to be my_var=pre_var). 
If there is white space in between the variable name, equal sign and the value, it will throw an error. 
So to define a function called my_func with a default argument zz with the value 98, yy with 12 we do 

(define (my_func xx yy=12 zz=98) 
	body)

To call a this function with yy as a keyword argument with the value 10
(my_func 7 zz=52 yy=47)

We handled error concerning default argument and keyword argument 
such as positonal argument follows keyowrd argument or  non-default argument follows default argument. Our example programs are in the example folder in a 
folder called scripts_for_testing_keyword_default_args. The programs specify exactly what they are testing in their comments. 


We are working on deliverable 7
...
or 4. For example,
https://github.com/MatthewGoin/SlytherLisp.git
Running D1 tests::
